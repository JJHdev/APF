...
(function($){
	...
	function loadBox(target, url, params ) {
		...
		var downloadUrl    = opts.url.downloadUrl
		...
		$.ajaxUtil.ajaxLoad(url, params, function(result) {
			...
			if (rows &&
				...
				$.each(rows, function(i, row) {
					var box = createBox(target, i, $.extend({},params,row));
					tdom.find('.'+opts.areaWrap).append( box );
					if(opts.canDownload) {
						box.find('input[name="fileName"]').on('click', function() {
						...
						box.find('input[name="fileName"]').css('cursor', 'pointer');
					}
				});
			} else {
				// 기본선택박스 생성
				var box = createBox(target, 0, params);
				tdom.find('.'+opts.areaWrap).append( box );
				if(opts.canDownload) {
					box.find('input[name="fileName"]').on('click', function() {
						download(target, downloadUrl, {
	...
	function select(target, args) {
		...
		$.ajaxUtil.ajaxLoad(loadUrl, params, function(result) {
			...
			if (rows &&
				...
				tdom.html('');
				$.each(rows, function(i,row) {
					var r = $('<div class="value-box btnFileDown app-pointer"></div>');
					r.append('<i class="icon-paperclip"></i>');
					r.append(row['fileNm']);
