...
*** ---------------------------------------------------------------------------
*** 1.0         2023.08.04              JH			시스템관리 서류코드관리 파일(조회,생성,삭제,수정 작업)
*******************************************************************************
**/
//===========================================================================//
// 업무첨부파일 컨트롤
...
(function($){
	...
	function createBox(target, index, data) {
		...
		if (opts.multiple) {
			row.append(
				...
				.append('<button class="px-12px '+opts.addBtn+' '+opts.clsOptions.btnCls+'"><i class="icon-plus"></i></button>')
			);
			row.append(
				$('<div class="col flex-grow-0 white-space-nowrap"></div>')
				.append('<button class="px-12px '+opts.delBtn+' '+opts.clsOptions.btnCls+'"><i class="icon-trash"></i></button>')
			);
		}
	...
	function loadBox(target, url, params ) {
		...
		// 2023.06.01 LSH 재사용을 위해 처리모드 제외함
	...
	$.fn[WIDGET].defaults = {
		...
		// 기본 URL
		url: {
			loadUrl        : getUrl('/'+SYSTEM.CODE.toLowerCase()+'/file/getListPapeFormFile.do'     ), // 파일 목록 조회 URL
			removeUrl      : getUrl('/'+SYSTEM.CODE.toLowerCase()+'/file/deletePapeFormFile.do'      ), // 파일 단일 삭제 URL
			uploadUrl      : getUrl('/'+SYSTEM.CODE.toLowerCase()+'/file/uploadPapeFormFile.do'      ), // 파일 업로드 URL
			downloadUrl    : getUrl('/'+SYSTEM.CODE.toLowerCase()+'/file/downloadPapeFormFile.do'    )  // 파일 다운로드 URL
		},
