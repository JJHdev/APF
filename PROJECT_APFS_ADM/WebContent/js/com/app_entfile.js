...
(function($){
	...
	function createBox(target, index, data) {
		...
		if (opts.downbtn) {
			...
			// 파일버튼
			let fbtn = $('<label class="'+opts.fileBtn+' '+opts.clsOptions.btnCls+'"><i class="icon-paperclip"></i></label>')
	...
	function select(target, args) {
		...
		let _getBtn = function(r) {
			...
			// 심플형 스타일인 경우
				...
				btn.append('<a href="javascript:void(0)" class="bs-s icon-file-pdf"></a>');
				btn.append(r['fileNm']);
			}
			// 박스형 스타일인 경우
			else {
