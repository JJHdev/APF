...
$.fn.appPopup = function ( args ) {
	...
	this.create = function() {
		thisElm.addClass("custom-modal fade");
		thisElm.prop("tabindex", "-1");
		thisElm.prop("aria-hidden", "true");
		thisElm.append('<div class="custom-modal-dialog"></div>');
		thisElm.find('.custom-modal-dialog').append('<div class="modal-content"></div>');
		if (options.style)
			thisElm.find('.custom-modal-dialog').prop('style', options.style);
		...
		if (options.buttons &&
			...
			let wrp = $('<div class="bottom-btn-box"></div>');
			wrp.append(thisCmp.createButtons(options.buttons));
			div.append(wrp);
			thisElm.find('.modal-content').append(div);
		}
		
		thisElm.find("button.close").bind('click', thisCmp.close);
		
		if (options.cls)
			thisElm.addClass(options.cls);
		if (options.dialogCls)
			thisElm.find('.custom-modal-dialog').addClass(options.dialogCls);

		if (options.appendBody) {
			//thisElm.appendTo('body');
			thisElm.appendTo(options.appendBody);
		}
	...
	this.createTitle = function() {
		...
		div.append('<p class="modal-title txt1"></p>');
		div.append('<button type="button" class="close" data-bs-dismiss="" aria-label="Close"><i class="icon-times"></i></button>');
		if (options.icon) {
			div.find('.modal-title').append(options.icon);
		}
		div.find('.modal-title').append(options.title);
...
$.fn.appMessage = function ( args ) {
	...
	// 팝업레이아웃 생성
	this.create = function() {
		thisElm.addClass("modal fade");
		thisElm.prop("tabindex", "-2");
		thisElm.prop("aria-hidden", "true");
		thisElm.append('<div class="modal-dialog custom-modal design1"></div>');
		thisElm.find('.modal-dialog').append('<div class="modal-content" style="max-width:900px"></div>');
		thisElm.find('.modal-content').append(thisCmp.createTitle());
		thisElm.find('.modal-content').append(thisCmp.createInner());
		if (options.cls)
			thisElm.addClass(options.cls);
		
		thisElm.modal({backdrop: 'static', keyboard: false}) ;		
			
		thisElm.on('shown.bs.modal', function() {
			$('#btn_msg_ok').focus();
			if (options.onshown) {
				options.onshown(thisCmp);
			}
		...
		if (options.onhidden) {
			thisElm.on('hidden.bs.modal', function() {
				options.onhidden(thisCmp);
			});
		}
		thisElm.appendTo('body');
	...
	// 팝업제목 생성
	this.createTitle = function() {
		let btn = $('<button type="button" class="close" data-bs-dismiss="modal" aria-label="Close"><i class="icon-times"></i></button>');
		btn.bind('click', this.close);
		let div = $('<div class="modal-header"></div>');
		div.append('<p class="modal-title txt1">'+options.title+'</p>');
		div.append(btn);
		return div;
	};
	...
	this.createButtons = function( buttons ) {
		...
		let div = $('<div class="bottom-btn-box"></div>');
...
******************************************************************************************
*** 파일명     : comm_popup.js
*** 설명      : 지원사업관리-신청현황관리용 팝업창 설정
***
*** -----------------------------    Modified Log   --------------------------------------
*** ver             date                author                  description
*** --------------------------------------------------------------------------------------
*** 1.0         2023.09.19              JH					지원사업관리-신청현황관리용 팝업창 설정
******************************************************************************************
**/
//===========================================================================//
// 공통팝업
//===========================================================================//
$.fn.appPopupFile = function ( args ) {
	...
	this.create = function() {
		...
		thisElm.prop("tabindex", "-1");
//		thisElm.prop("aria-hidden", "true");
		thisElm.append('<div class="custom-modal-dialog" style="width:970px; transform: scale(0.8); transform-origin:24% -400%;"></div>');
		thisElm.find('.custom-modal-dialog').append('<div class="modal-content"></div>');
		if (options.style)
			thisElm.find('.custom-modal-dialog').prop('style', options.style);
		thisElm.find('.modal-content').append(thisCmp.createTitle());
...
$.fn.appMessage = function ( args ) {
	...
	this.create = function() {
		...
		thisElm.prop("tabindex", "-1");
		thisElm.prop("aria-hidden", "true");
		thisElm.append('<div class="custom-modal-dialog" style="width:500px"></div>');
		thisElm.find('.custom-modal-dialog').append('<div class="modal-content"></div>');
		thisElm.find('.modal-content').append(thisCmp.createTitle());
		thisElm.find('.modal-content').append(thisCmp.createInner());
		if (options.cls)
	...
	this.createButtons = function( buttons ) {
		...
		$.each(buttons, function(i,b) {
			...
			let col = $('<div class="col"></div>');
			let obj = $('<button type="button" class="app-msg-click w-30 bs-l" style="outline:none"></button>');
