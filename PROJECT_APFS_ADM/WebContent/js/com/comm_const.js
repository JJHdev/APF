...
// 대메뉴 아이콘
const MENU_ICONS = {
	 MU_ADM_MAN: 'icon-grid-web-4'       // 대시보드
	,MU_ADM_MEM: 'icon-user-edit'        // 회원관리
	,MU_ADM_SUP: 'icon-presentation-pen' // 지원사업관리
	,MU_ADM_INV: 'icon-credit-card-edit' // 투자정보관리
	,MU_ADM_MNG: 'icon-notebook'         // 운영관리
	,MU_ADM_BBS: 'icon-layout-web-12'    // 게시판관리
	,MU_ADM_SVY: 'icon-pen-tool'         // 설문관리
	,MU_ADM_SYS: 'icon-settings'         // 시스템관리
};
...
//편집 모드
const YOUTUBE = {
	API_KEY:  'AIzaSyDWpWZixLN90ShzmoFeH9IFAfJgVHEWUaM',
};
...
const CODES = {
	...
	// 지원단계 목록
	SPRT_STEP_LIST: [
		{code : 'CHOS', icon: 'icon-cursor-click'        , text : '지원사업 선택', filter: function(key) { return (key != CODE.SPRT_APLY_SE.BEFORE); }},
		{code : 'AGRE', icon: 'icon-clipboard-list-check', text : '약관 수집 동의'},
		{code : 'FORM', icon: 'icon-pen-line'            , text : '신청서 작성'},
		{code : 'FILE', icon: 'icon-file-text-edit'      , text : '제출서류'},
...
const OPTIONS = {
	...
	DATEPICKER_NOT_TOMORROW: {
	    format: 'yyyy-mm-dd', //데이터 포맷 형식(yyyy : 년 mm : 월 dd : 일 )
	    autoclose: true, //사용자가 날짜를 클릭하면 자동 캘린더가 닫히는 옵션
	    endDate: '0d', // 오늘 날짜 이후를 선택할 수 없게 하는 옵션
	    calendarWeeks: false, //캘린더 옆에 몇 주차인지 보여주는 옵션 기본값 false 보여주려면 true
	    templates: {
	       leftArrow: '<i class="icon-angle-left"></i>',
	       rightArrow:'<i class="icon-angle-right"></i>',
	...
	DATEPICKER_MONTH: {
	    ...
	    language: 'ko', //달력의 언어 선택, 그에 맞는 js로 교체해줘야한다.
		changeMonth: true,   // 월을 변경할 수 있도록 설정
    	changeYear: true,    // 년도를 변경할 수 있도록 설정
...
const VALIDATE = {
	...
	,phone   : {name:'전회번호', ex: '02-1234-5678'}
	...
	,phoneNo : {name:'전회번호', ex: '02-1234-5678'}
...
const BZENTY_TYPE_FILTER = {
	getBzentyType: function(bzentyType) {
		if(bzentyType == '20')
			return CODE.PAPE_TYPE.ENT
		return CODE.PAPE_TYPE.USR
	}
...
// 23.09.05 LHB 검색은 submit 하지 않는 관계로 validation 함수 별도로 선언
// 검증함수
const CONDITION = {
	yearCheck: function validateDate(before, after, message) {
		if (!before || !after) {
			return true;
		}
