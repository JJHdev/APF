...
$(function() {
	...
	P_RO_GRID.appBbsGrid({
		...
		title:		'권한 적용 프로그램 목록',
		// 검색 URL
		url:		getUrl('/adm/sys/role/getListRoleProg.do'),
        // 조회 파라미터 전달
		...
		listOptions: {
			...
			columns: [
	            ...
	            {name:'prgrmId'       ,cls:'app-c' ,label:'프로그램ID'},
	        ],
		},
	}).appBbsGrid('init');
	...
	P_ME_GRID.appBbsGrid({
		...
		title:		'추가 대상 프로그램 목록',
		// 검색 URL
		url:		getUrl('/adm/sys/role/getListNotRoleProg.do'),
        // 조회 파라미터 전달
		queryParams: { srchRoleId:'ROLE_ADM_MNG'},
	    //목록 검색 페이징 객체
		pagination: { display: 20 },
    ...
    function doRemove() {
        ...
        if (rows.length == 0) {
            $.commMsg.alert('제외할 프로그램을 선택하세요.');
            return false;
        }
        $.commMsg.confirm(rows.length+"개의 프로그램을 제외하시겠습니까?", function() {
            // AJAX로 저장처리
            $.ajaxUtil.ajaxSave(
                getUrl('/adm/sys/role/saveRoleProg.do'), 
                JSON.stringify({
                    mode: MODE.REMOVE,
                    roleList: rows
                }),
    ...
    function doAppend() {
        ...
        $.commMsg.confirm(rows.length+"개 프로그램을 추가하시겠습니까?", function() {
            // AJAX로 저장처리
            $.ajaxUtil.ajaxSave(
                getUrl('/adm/sys/role/saveRoleProg.do'), 
                JSON.stringify({
