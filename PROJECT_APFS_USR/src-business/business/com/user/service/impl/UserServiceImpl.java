...
public class UserServiceImpl extends BaseService implements UserService {
	...
	/**
	 * 기업회원 권한변경처리 (대표계정에 한함)
	 * - 권한정보 리셋
	 * - 그룹정보 리셋
	 */
	private void _changeRole(String userNo, String roleId) throws Exception {
		
		// ROLE정보 재등록
		userDAO.deltUserRole(userNo);
		userDAO.regiRole(
			UserVO.builder()
			.userNo(userNo)
			.roleId(roleId)
		...
		// 그룹정보 재등록
		groupService.updtGrpRprs(userNo, roleId);
	}
	...
	public int saveEntCmpl(UserInfo user, EntVO entVO) throws Exception {
		...
		if (!CommUtils.isEqual(orgSeCd, newSeCd)) {
			// 권한변경처리
			_changeRole(userNo, roleId);
		}
		// 업체정보 수정처리
		return entService.saveEntCmpl(entVO);
	}
	...
	public List<UserVO> listUserByEnt(String bzentyNo) throws Exception {
    	return userDAO.listUserByEnt(bzentyNo);
    }
}
