...
public class CommBizUtils {
    ...
    /**
     * 2023.09.22 LSH
     * 국가번호가 붙은 휴대폰번호를 국가번호를 제외한 기본 형태로 변환처리
     * 입력예제: +82 10-0000-0000
     * 출력예제: 01000000000
     */
    public static String toMobileNumber(String number) {
    	...
    	if (number.indexOf(prefix) == 0) {
    		return p.matcher("010"+number.substring(prefix.length())).replaceAll("");
    	}
    	return p.matcher(number).replaceAll("");
    }
}
