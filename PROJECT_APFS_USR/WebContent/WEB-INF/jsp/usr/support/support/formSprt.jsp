...
<form id="selectForm" name="selectForm" method="post" onsubmit="return false;">
	...
	<input type="hidden" id="sprtAplyNo"   name="sprtAplyNo"   value="<c:out value="${model.sprtAplyNo  }"/>"/><%-- 지원신청번호 --%>
	<input type="hidden" id="bzentyNo"     name="bzentyNo"     value="<c:out value="${model.gsBzentyNo  }"/>"/><%-- 신청업체번호 --%>
	<input type="hidden" id="userNo"       name="userNo"       value="<c:out value="${model.gsUserNo    }"/>"/><%-- 신청사용자번호 --%>
</form>
