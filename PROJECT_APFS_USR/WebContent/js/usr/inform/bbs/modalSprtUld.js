...
******************************************************************************************
*** 파일명 : modalSprtUld.js
*** 설명글 : 정보서비스-경영체 데이터 업로드의 파일업로드 화면 스크립트
***
*** -----------------------------    Modified Log   --------------------------------------
...
$(function() {
    ...
    function doSave() {
		...
		$.commMsg.confirm("업로드하시겠습니까?", function() {
            // 등록폼을 AJAX로 저장처리
            P_RFORM.ajaxForm({
                url: getUrl('/usr/inform/bbs/saveSprtUld.do'),
                // 오류시 처리로직
	...
	// 파일 업로드 폼 초기화
	P_FILE_BOX.clearForm();
	
	// 업로드 결과 팝업 오픈
    //--------------------------------------------------------//
	function openModal(params) {
		P_MODAL2 = $('<div></div>').appPopupSprtUldResult(params
		, function() {
			return false;
		});
	}
...
$.fn.appPopupSprtUldResult = function( params, saveCallback ) {
	...
	let options = {
		...
		loadUrl			: getUrl('/usr/inform/bbs/modalSprtUldResult.do'),
	};
