...
$(function() {
    ...
    P_GRID.appBbsGrid({
		...
		title:      '유관기관 커뮤니티',
		// 검색 URL
		url:         getUrl('/usr/inform/bbs/getListCommunity.do'),
		//목록 검색 페이징 객체
		...
		listOptions: {
			...
			emptyText: emptyText(),
			// 목록칼럼 너비
			colgroup: ['78px','502px','260px','150px','78px'],
			// 목록칼럼 정의
			columns: [
    ...
    function doSelect(row) {
        ...
        $.formUtil.submitForm( getUrl('/usr/inform/bbs/viewCommunity.do'), {
			params: {
				pstNo     : row['pstNo'  ],
				bbsSeCd   : row['bbsSeCd'],
...
// [팝업] 글작성 팝업
//=============================================================================
$.fn.appPopupCommunity = function( params, saveCallback ) {
	...
	let options = {
		title     : '유관기관 커뮤니티 글 작성하기',
		icon      : '<img src="'+getUrl('/images/sub/Sketch_on_smartphone.svg')+'">',
		loadUrl   : getUrl('/usr/inform/bbs/modalCommunityForm.do'),
		loadParams: {},
	};
	...
	return $(this).appPopup({
		...
		onloaded:   function(pop) {
			...
			$('#p_btnSave').bind('click', function() {
		    	...
		    	$.commMsg.confirm('유관기관커뮤니티 글을 등록하시겠습니까?', function() {
			
					// 등록모드 설정
					pform.find('[name="mode"]').val(MODE.INSERT);
		            // 등록폼을 AJAX로 저장처리
		            pform.ajaxForm({
		                url: getUrl('/usr/inform/bbs/saveCommunity.do'),
