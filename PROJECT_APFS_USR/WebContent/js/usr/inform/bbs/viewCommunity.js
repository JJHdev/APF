...
$(function() {
    ...
    function doRemove() {
        $.commMsg.confirm("정말 삭제하시겠습니까?", function() {
            // AJAX로 삭제처리
            $.ajaxUtil.ajaxLoad(
                getUrl('/usr/inform/bbs/saveCommunity.do'), 
                {pstNo   : P_PST_NO,
				 bbsSeCd : P_BBS_CD,
                 mode    : MODE.REMOVE
                }, 
    ...
    //--------------------------------------------------------//
    function goList() {
           $.formUtil.submitForm(getUrl("/usr/inform/bbs/listCommunity.do"), {
               formId : "searchForm"
           });
        return false;
    }
  ...
  $.fn.appPopupData = function( params, saveCallback ) {
  	...
  	return $(this).appPopup({
  		...
  		onloaded:   function(pop) {
  		    ...
  		    //========================================================//
  		    // 웹에디터 정의 (CKEDITOR 초기화 (comm_ckeditor.js 참고))
  		    //--------------------------------------------------------//
  			...
  			$('#p_btnSave').bind('click', function() {
  		    	...
  		    	$.commMsg.confirm('유관기관 커뮤니티 글을 수정하시겠습니까?', function() {
