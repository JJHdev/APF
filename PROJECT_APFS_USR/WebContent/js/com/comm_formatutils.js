...
// 음수포함숫자만 입력 이벤트
function bindOnlyMinusNumber( box ) {
	box.bind('keyup', function(e) {
		$(this).val($(this).val().replace(/[^-0-9]/g, ""));
	});
}
 ...
 * 2023.06.15 LSH 추가
 * 2023.08.17 LSH 전체를 제외한 element 선택되는 경우 전체선택처리 추가
 */
...
$.formatUtil = {
	...
	// 원단위를 억단위로 환산처리
	toBillion: function( s ) {
		let dvd = 100000000;
		let num = $.commUtil.nvlInt(s);
		if (num % dvd === 0)
			num = num / dvd;
		else
			num = (num / dvd).toFixed(1);
		let wrd = STRING.UNIT.BKRW;
		return num+wrd;
	},
	toInvtHopeAmtFmt: function(n) {
		let num = $.commUtil.nvlInt(n) / 100;
		let wrd = STRING.UNIT.BKRW;
		return '<em class="fs-20px text-primary">'+num+'</em>'+wrd;
	},
	toInvtHopeAmt: function(n) {
		return {
			digits: $.commUtil.nvlInt(n) / 100,
			word  : STRING.UNIT.BKRW
		};
		//let s   = $.formatUtil.toInvtHopeWon(n);
	...
	toInvtHopeWon: function( number, words ) {
		
		words = words || ['억','조']
		
	    var inputNumber  = number < 0 ? false : number / 100;

		let remain = inputNumber;
		let word = words[0];
		for (let i=0; i<words.length; i++) {
			remain = Math.floor(inputNumber / Math.pow(1000, [words.length - i - 1]));
			if (remain > 0) {
	...
	toFundScale: function(n) {
		return '<em class="fs-20px text-primary">'+$.formatUtil.toNumber(n)+'</em>'+STRING.UNIT.BKRW;
	},
	...
	// 2023.08.25 숫자포맷 + '천원'
	toUnitKilo: function( s ) {
		return $.formatUtil.toNumber(s)+' <em>'+STRING.UNIT.KKRW+'</em>';
	},
	// 2023.08.25 숫자포맷 + '백만원'
	toUnitMillion: function( s ) {
		return $.formatUtil.toNumber(s)+' <em>'+STRING.UNIT.MKRW+'</em>';
	},
	// 2023.08.25 숫자포맷 + '억원'
	toUnitBillion: function( s ) {
		return $.formatUtil.toNumber(s)+' <em>'+STRING.UNIT.BKRW+'</em>';
	},
	// yyyymmdd를 yyyy.mm
	toDotYm: function( s ) {
		if ($.commUtil.empty(s))
	...
	toPreHtml: function( s ) {
		let d = $('<div class="app-pre"></div>');
		d.append( $.commUtil.restoreHtml(s) );
		return d;
	}
};
