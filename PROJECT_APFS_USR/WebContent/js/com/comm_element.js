...
$.fn.appSquareBox = function ( args ) {
	...
	//값 INDEX 선택
	this.setClick = function(index) {
		thisElm.find('.'+options.boxCls).each(function(i) {
			if (index == i) {
				thisCmp.onClick($(this));
				return true;
			}
...
$.fn.appSelectBox = function ( args ) {
	...
	this.createBox = function(index, boxOpts) {
		...
		if (options.labelCls)
			wrap.find('label').addClass(options.labelCls);
		
		if (tooltip) {
			let tip = $('<span class="toltip"></span>');
			tip.append('<em class="close icon-times-F"></em>');
			tip.append('<p>'+label+'</p>');
			tip.append('<span class="app-tip"></span>');
			if (options.tooltipCls)
				tip.find('.app-tip').addClass(options.tooltipCls);
			tip.find('.app-tip').append($.commUtil.restoreHtml(tooltip));
	...
	this.create = function( data ) {
		...
		$.each(rows, function(i,row) {
			...
			if (i == 0) {
				...
				r.find('em').prop('style', 'margin-left:-20px');
			}
...
$.fn.appGridRadioBox = function ( args ) {
	...
	var thisName = Math.random().toString(36).substr(2,11);
