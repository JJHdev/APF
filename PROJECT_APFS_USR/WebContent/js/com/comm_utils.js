 ...
 */
$.treeUtil = {
	// 2023.08.24 LSH 계층구조 배열의 가장 최종객체를 삭제한다.
	deleteLast( arr ) {
		for (let i = (arr.length-1); i >= 0; i--) {
			if (arr[i].items) {
				if ($.treeUtil.deleteLast(arr[i].items))
					return true;
			}
	...
	// 2023.08.24 LSH 계층구조 배열의 가장 최종객체를 반환한다.
	findLast( arr ) {
		for (let i = (arr.length-1); i >= 0; i--) {
			let o = arr[i];
			if (o.items)
				return $.treeUtil.findLast(o.items);
			else if (i == (arr.length-1))
				return o;
		}
	}
