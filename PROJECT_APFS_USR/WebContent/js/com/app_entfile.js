...
(function($){
	...
	function createArea(target) {
		...
		if (opts.message) {
			...
			btm.find('.col').append('<p></p>');
			btm.find('p').addClass(opts.messageColor);
			btm.find('p').append(opts.message);
			if (opts.messageCls)
				btm.find('p').addClass(opts.messageCls);
			tdom.append(btm);
		}
	};
	...
	function createCheck(target) {
		...
		// 내용체크박스
		let chk = $('<input/>');
		chk.prop('type' , opts.check.type || 'checkbox');
	...
	// 부가항목 생성 (옵션) - docuYn에 맵핑됨
	function createExtra(target, index, data) {
		...
		// 내용체크박스
		let chk = $('<input/>');
		chk.prop('type' , opts.extra.type || 'checkbox');
		chk.prop('id'   , opts.extra.name+index);
		chk.prop('name' , opts.extra.name);
		chk.prop('value', data['fileNo']);
		...
		chk.bind('click', function() {
			if ($(this).is(':checked')) {
				tdom.find('input[name="upfile"]').each(function(i) {
					tdom.find('input[name="docuYn"]').eq(i).val('N');
				});
				$(this).closest('.'+opts.fileWrap).find('input[name="docuYn"]').val('Y');
			}
		});
		// 내용레이블
		let lbl = $('<label for="'+opts.extra.name+index+'"></label>');
		lbl.append(opts.extra.label);
		if (opts.extra.icon) {
			lbl.append('<i class="'+opts.extra.icon+'"></i>');
		}
		let box = $('<div class="check-radio-box d-block"></div>');
		box.append(chk);
		box.append(lbl);
		return box;
	};
	...
	function createBox(target, index, data) {
		...
		ele.append('<input type="file"   name="upfile"   class="'+opts.fileBox+'">');
		row.append($('<div class="col"></div>').append(ele));
		
		// 수정시 다운로드버튼 표시인 경우
		if (opts.downbtn) {
			// 다운로드버튼
			let btn = $('<button type="button" class="'+opts.clsOptions.btnCls+'"><i class="icon-download"></i></button>');
			btn.data('type', data['fileType']);
			btn.data('no'  , data['docuNo'  ]);
			btn.data('seq' , data['fileNo'  ]);
			btn.on('click', function() {download(target, false, {sn: btoa($(this).data('seq'))});});
			row.append($('<div class="col flex-grow-0 white-space-nowrap '+opts.btnWrap+'"></div>').append(btn));
		}
		else {
			// 파일버튼
			...
			// 부가항목표시인 경우
			if (opts.extra) {
				row.append($('<div class="col flex-b108px app-file-extra"></div>').append(createExtra(target, index, data)));
			}
		}
		// 양식다운로드 버튼 표시인 경우
		if (opts.formbtn) {
			...
			doc.data('code', data['docuCd']);
			doc.addClass(opts.clsOptions.btnCls);
			doc.addClass(opts.papeBtn);
			doc.append('<i class="icon-download"></i>');
			doc.append('양식다운로드');
			row.append( $('<div class="col flex-grow-0 white-space-nowrap"></div>').append(doc) );
		}
		
		let box = $('<div class="'+opts.clsOptions.areaCls+' mb-16px '+opts.fileWrap+'"></div>');
		if (index == 0) {
			if (!opts.check && opts.label) {
				let lbl = $('<label class="app-file-label">'+opts.label+'</label>');
				if (opts.labelCls)
					lbl.addClass(opts.labelCls);
				box.append(lbl);
			}
		}
	...
	function bindBox(target, elm) {
		...
		elm.find(fadd).click(function() {
			...
			// 부가항목표시인 경우
			if (opts.extra) {
				let fid = Math.random().toString(36).substr(2,11);
				cform.find('.app-file-extra input[type="radio"]').prop('id', fid);
				cform.find('.app-file-extra label').prop('for', fid);
			}
		...
		elm.find(fdel).click(function() {
			...
			if ($.commMsg.confirm('파일을 삭제하시겠습니까?')) {
				fnDelete();
				return false;
			}
			return false;
		...
		// 양식파일 다운로드 이벤트 바인딩
		elm.find(fpape).click(function() {
			// 양식파일 다운로드 실행
			download(target, 
				opts.url.downloadPapeUrl,
				{fileType: $(this).data('code')}
			);
			return false;
		});
	};
	...
	function select(target, args) {
		...
		// 파일다운로드 버튼 생성
		let _getBtn = function(r) {
			let btn = false;
			// 목록형 스타일인 경우
			if (opts.screen == 'LIST') {
				btn = $('<a href="javascript:void(0)" class="bs-s btn-primary px-6px mx-5px"><i class="text-white icon-download"></i></a>');
				btn.addClass(opts.listOptions.btnCls);
			}
			// 심플형 스타일인 경우
			else if (opts.screen == 'SIMPLE') {
				btn = $('<div class="app-pointer my-2px"></div>');
				btn.append('<a href="javascript:void(0)" class="bs-s btn-primary icon-download px-6px mx-5px"></a>');
				btn.append(r['fileNm']);
			}
			// 박스형 스타일인 경우
			else {
				btn = $('<button type="button"><i class="icon-download"></i></button>');
				btn.addClass(opts.boxOptions.btnCls);
			}
			btn.data('type', r['fileType']);
			btn.data('no'  , r['docuNo'  ]);
			btn.data('seq' , r['fileNo'  ]);
		...
		// 라인단위 파일 항목 반환
		let _getLine = function(r) {

			// 심플형 스타일인 경우
			if (opts.screen == 'SIMPLE') {
				return _getBtn(r);
			}
			// 목록형 스타일인 경우
			else if (opts.screen == 'LIST') {
				return $.domUtil.getRow({
					items: [
						 {cls:    false, formatHtml: r['fileNm']}
						,{cls: 'btnbox', formatHtml: _getBtn(r)}
					]
				});
			} 
			// 박스형 스타일인 경우
			return $.domUtil.getRow({
				items: [
					 {cls:'iconbox', formatHtml: '<i class="icon-file-pdf"></i>'}
					,{cls:    false, formatHtml: r['fileNm']}
					,{cls: 'btnbox', formatHtml: _getBtn(r)}
				]
			});
		...
		$.ajaxUtil.ajaxLoad(loadUrl, params, function(result) {
			...
			if (rows &&
				...
				rows.length > 0) {
				tdom.html('');
				// screen 옵션에 따라 스타일을 다르게 처리함
				// 목록형 스타일인 경우
				if (opts.screen == 'LIST') {
					tdom.addClass(opts.listOptions.wrapCls);
					
					let table = $('<table></table>');
					table.append('<tbody class="app-file-list"></tbody>');
					table.find('tbody').addClass(opts.listOptions.tbodyCls);

					$.each(rows, function(i,row) {
						let tr = $('<tr></tr>');
						tr.addClass(opts.listOptions.trCls);
						tr.append($('<td class="'+opts.listOptions.thCls+'"></td>').append( _getLine(row)));
				...
				// 심플형 스타일인 경우
					...
					let box = $('<div class="row app-file-list"></div>');
					$.each(rows, function(i,row) {
						box.append($('<div class="col-12"></div>').append( _getLine(row) ));
					});
					tdom.append(box);
				}
				// 박스형 스타일인 경우
				else {
					let box = $('<div class="info-filelistbox"></div>');
					box.append('<div class="row app-file-list"></div>');
					$.each(rows, function(i,row) {
						let col = $('<div class="info-filelist shadow-box-1"></div>');
						col.append( _getLine(row) );
						box.find('.app-file-list').append($('<div class="col-12 mb-16px"></div>').append(col));
					});
					tdom.append(box);
				}
			}
				...
				if (opts.emptyText) {
					tdom.append($.domUtil.getEmptyBox(opts.emptyText, 'mt-24px'));
				}
			}
		});
	};
		...
		tdom.find('input[name="upfile"]').each(function(i) {
			...
			if (nfile == '' && ofile == '') {
				...
				if (need != 'Y')
					return true;
				if (isAlert)
					$.commMsg.alert(title+'파일을 선택해 주세요.');
				check = false;
				return false;
			}
			// 파일이 있는 경우
	...
	$.fn[WIDGET].defaults = {
		...
		// 기본 URL
		url: {
			loadUrl        : getUrl('/'+SYSTEM.CODE.toLowerCase()+'/file/getListEntFile.do'     ), // 파일 목록 조회 URL
			removeUrl      : getUrl('/'+SYSTEM.CODE.toLowerCase()+'/file/deleteEntFile.do'      ), // 파일 단일 삭제 URL
			uploadUrl      : getUrl('/'+SYSTEM.CODE.toLowerCase()+'/file/uploadEntFile.do'      ), // 파일 업로드 URL
			downloadUrl    : getUrl('/'+SYSTEM.CODE.toLowerCase()+'/file/downloadEntFile.do'    ), // 파일 다운로드 URL
			downloadPapeUrl: getUrl('/'+SYSTEM.CODE.toLowerCase()+'/file/downloadEntFormFile.do')  // 양식파일 다운로드 URL
		},
		...
		listOptions: {
			...
			tdCls: '',
			// 버튼 CLS
			btnCls: 'bs-m btn-primary-ouline px-8px'
		},
		// 심플형 경우 설정옵션
		simpleOptions: {
		},
