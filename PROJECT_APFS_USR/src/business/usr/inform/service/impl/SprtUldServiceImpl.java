...
import business.usr.support.service.impl.SprtBizDAO;
...
/**
 * [서비스클래스] - 정보서비스-경영체 데이터 업로드 Service 구현 클래스
 * 
 * @class   : SprtUldServiceImpl
 * @author  : LHB
 * @since   : 2023.07.13
 * @version : 1.0
 *
...
public class SprtUldServiceImpl extends BaseService implements SprtUldService {
	...
	public HashMap saveSprtUld(SprtBizVO sprtBizVO, List<FileInfo> files) throws Exception {
		...
		if (CommConst.INSERT.equals(mode)) {
			...
			for (Map data : dataList) {
	    		...
	    		// 소속기관 동일한지 체크
				List<String> crdnsNoErrors = ce.validateCrdnsNoErrors(data, (String) crdnsMap.get("cdId"));
				if (crdnsNoErrors != null) {
					throw processException("error.adm.sprt.notCrdnsNo", new String[] { String.valueOf(sn), crdnsNoErrors.toString() });
				}
				// 필수값 입력 체크
	...
	/**
     * 정보서비스-경영체 데이터 업로드 - 정부지원사업이력 페이징목록조회
     */
    @Override
	...
	public String saveSprtBiz(SprtBizVO sprtBizVO, long[] arrSn) throws Exception {
		...
		for(long sn : arrSn) {
			sprtBizVO.setSn(sn);
			data = sprtBizDAO.viewSprtBiz(sprtBizVO);
