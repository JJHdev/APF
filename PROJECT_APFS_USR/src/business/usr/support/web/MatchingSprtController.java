...
public class MatchingSprtController extends BaseController {
    
    @Resource(name="MatchingService")
    protected MatchingService matchingService;

    /**
     * 지원서비스-매칭서비스-매칭설정 화면 오픈
     * 지원서비스 내의 URL로 변경함
     * 
     * 화면 표시 조건
     * 1) showMode = 'X' : 비로그인시
     * 2) showMode = 'N' : 로그인 - 매칭미설정시
     * 2) showMode = 'B' : 로그인 - 관리자/경영체/유관기관/농금원인 경우
     * 3) showMode = 'U' : 로그인 - 개인회원인 경우
     */
    @RequestMapping("/usr/support/matching/openMatchingSprt.do")
    public String openMatchingSprt(ModelMap model
        ...
        if (CommConst.isLogin(gsUserNo)) {
        	...
        	// 관리자/경영체/유관기관/농금원인 경우
        	else if (CommConst.isAdminRole(gsRoleId) ||
        			 CommConst.isEntRole(gsRoleId) ||
        ...
        model.addAttribute("mtchMode", CommConst.MTCH_MODE_SPRT );
    	
        return "usr/support/matching/openMatchingSprt";
    }
}
