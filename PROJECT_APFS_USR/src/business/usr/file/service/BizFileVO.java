...
public class BizFileVO extends BaseModel implements FileVO {
    ...
    // 파일목록
    private List<BizFileVO> files;

    public void addFiles(BizFileVO fileVO) {
    	if (files == null)
    		files = new ArrayList<BizFileVO> ();
    	files.add(fileVO);
    }
	...
	public void buildPath(BaseModel modelVO) {
		...
		switch (getTaskSeCd()) {
			...
			case CodeConst.TASK_SPRT : 
				SprtVO sprtVO = (SprtVO)modelVO;
		        setDocNo   (sprtVO.getSprtAplyNo());
		        setDtlDocNo(sprtVO.getBzentyNo  ());
