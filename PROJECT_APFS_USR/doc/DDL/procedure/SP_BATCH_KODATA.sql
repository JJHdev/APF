	...
	*/	
	RAISE NOTICE '특허및실용신안 등록 시작 = %',	  curr_time;
		...
		DELETE FROM TB_ENT_PTNT
		 WHERE 0 = 0
		   AND PATENT_SE_CD		= 'P11'
		   AND (IR_NO, KD_CD)	IN (
									SELECT DISTINCT C.IR_NO, A.KODATACD
									  FROM (
											SELECT KODATACD
												 , BASE_DT
		...
		UPDATE IF_50TH A
		   SET LINK_STUS 	= '-'
		 WHERE LINK_STUS 	= 'N'
		   AND (KODATACD, BASE_DT)  IN (
										SELECT DISTINCT A.KODATACD, A.BASE_DT
										  FROM (
												SELECT KODATACD
													 , BASE_DT
													 , MAX(LINK_STUS)	AS LINK_STUS
